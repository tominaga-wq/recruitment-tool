...
【重要】8社の選出は、採用ハードルスコア（H）を必ず考慮してバランスよく構成してください。
- H=1〜2（難関）の企業：2社程度（候補者のスキルが高い場合はチャレンジとして）
- H=3（中程度）の企業：3〜4社程度（本命として）
- H=4〜5（通過しやすい）の企業：2社程度（確実に内定を取りに行く枠として）
スキルマッチ（S）が高い企業だけを選ぶのではなく、H値の異なる企業を意図的に混ぜて選出してください。
...
【S採点の特別ルール：内定実績との類似性ボーナス】
採点時に提供する『各企業の過去内定者プロフィール』を必ず参照してください。
今回の候補者の経歴が、いずれかの内定者の経歴と類似性が高い場合、Sを通常より+1点高く評価してください（最大5点まで）。
その場合、match_reasonの中に「※内定実績との類似：〇〇（職種・年齢・経験年数など）と類似」と根拠を必ず記載してください。
