...
以下の【一次選考】→【二次選考】の2段階で上位8社を選出してください。

━━━━━━━━━━━━━━━━━━━━
【一次選考】57社 → 約20社に絞る
━━━━━━━━━━━━━━━━━━━━
各企業について以下の条件で除外し、残った企業（約20社）を二次選考に進めてください。

除外条件：
1. 【NG要件】に1つでも該当する企業は必ず除外
2. 【必須要件（Must）】を明らかに満たさない企業は除外（曖昧・判断が難しい場合は除外せず残すこと）

━━━━━━━━━━━━━━━━━━━━
【二次選考】約20社 → 上位8社を選出
━━━━━━━━━━━━━━━━━━━━
一次選考を通過した企業に対して採点し、以下のルールで8社を選出してください。

## 採点項目
- S（スキルマッチ）：経験・経歴の親和性と過去内定事例との類似度を総合評価。5=完全一致、1=ほぼ合致なし
- A（志向性の一致）：転職理由・希望する次のキャリア（業界・職種）・将来プランを総合的に判断した合致度。5=まさに求めている、1=全く合わない
- H_estimated（H未登録企業のみ）：必須要件・年齢制限・経験年数・学歴条件の厳しさからHを推定（1=最難関, 5=最も通りやすい）。H未登録企業はSを保守的に採点してください。
※ H登録済み企業についてはH_estimatedの出力不要です。

## 内定実績ボーナス
提供する『各企業の過去内定者プロフィール』を必ず参照してください。
候補者の経歴が内定者と類似する場合、Sを+1点（最大5点）。match_reasonに「※内定実績との類似：〇〇と類似」と根拠を記載してください。

## 8社の構成ルール
採用ハードルスコア（H）を参考に以下の比率で選出してください。
- H=1〜2（難関）：1〜2社（チャレンジ枠）
- H=3（中程度）：1〜3社（本命枠）
- H=4〜5（通過しやすい）：3〜4社（セーフティー枠）
スコアが同点の場合は、内定実績がある企業を優先してください。
