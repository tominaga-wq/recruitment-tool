...
【重要】8社の選出は、採用ハードルスコア（H）を必ず考慮して以下の構成で選出してください。
- H=1〜2（難関）の企業：1〜2社（チャレンジ枠）
- H=3（中程度）の企業：1〜3社（本命枠）
- H=4〜5（通過しやすい）の企業：3〜4社（セーフティー枠）
スキルマッチ（S）が高い企業だけを選ぶのではなく、H値の異なる企業を上記の比率で必ず混ぜて選出してください。
